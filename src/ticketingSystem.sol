...
contract TicketingSystem {
    ...
    // function cashOutConcert(uint _concertId, address payable _cashOutAddress) public{
    //     require(block.timestamp >= concertsRegister[_concertId].concertDate, "should be after the concert");
    //     require(_cashOutAddress == artistsRegister[concertsRegister[_concertId].artistId].owner, "should be the artist");
    //     uint256 totalMoneyCollected = concertsRegister[_concertId].totalMoneyCollected;
    //     uint256 venueCommission = venuesRegister[concertsRegister[_concertId].venueId].venueCommission;
    //     uint256 artistCommission = totalMoneyCollected - venueCommission;
    //     payable(venuesRegister[concertsRegister[_concertId].venueId].owner).transfer(venueCommission);
    //     payable(artistsRegister[concertsRegister[_concertId].artistId].owner).transfer(artistCommission);
    //     payable(_cashOutAddress).transfer(address(this).balance);
    // }

    // function cashOutConcert(uint _concertId, address payable _cashOutAddress) public{
    //     if (msg.sender == venuesRegister[concertsRegister[_concertId].venueId1].owner){
    //         uint256 totalMoneyCollected = concertsRegister[_concertId].totalMoneyCollected1;
    //         uint256 venueCommission = venuesRegister[concertsRegister[_concertId].venueId1].venueCommission;
    //         uint256 artistCommission = totalMoneyCollected - venueCommission;
    //         payable(venuesRegister[concertsRegister[_concertId].venueId1].owner).transfer(venueCommission);
    //         payable(artistsRegister[concertsRegister[_concertId].artistId1].owner).transfer(artistCommission);
    //         payable(_cashOutAddress).transfer(address(this).balance);
    //     }
    // }
    ...
    // function offerTicketForSale(uint _ticketId, uint _salePrice) public{
    //     if (msg.sender == ticketsRegister[_ticketId].owner){
    //         ticketsRegister[_ticketId].isAvailableForSale = true;
    //         ticketsRegister[_ticketId].amountPaid = _salePrice;
    //     }
    // }
    ...
    // function buySecondHandTicket(uint256 _ticketId) public payable{
    //     if (msg.value >= ticketsRegister[_ticketId].amountPaid){
    //         ticketsRegister[_ticketId].owner = payable(msg.sender);
    //         ticketsRegister[_ticketId].isAvailableForSale = false;
    //         ticketsRegister[_ticketId].amountPaid = msg.value;
    //     }
